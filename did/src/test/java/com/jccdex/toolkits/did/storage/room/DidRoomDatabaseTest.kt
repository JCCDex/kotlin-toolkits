...
class DidRoomDatabaseTest {
    ...
    fun getInstance_withoutName_usesDefaultDatabaseName() {
        val context = ApplicationProvider.getApplicationContext<Context>()
        dbName = DidRoomDatabase.DEFAULT_DATABASE_NAME

        val viaDefault = DidRoomDatabase.getInstance(context)
        val viaExplicit = DidRoomDatabase.getInstance(context, DidRoomDatabase.DEFAULT_DATABASE_NAME)
