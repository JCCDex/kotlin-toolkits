...
class NftStoreTest {
    ...
    @Test
    fun wrapperMethods_delegateToDaoAndObserveCollections() = runTest {
        ...
        try {
            ...
            assertThat(store.getEvmNftItemByContractAndTokenId("0x1", "0xabcdefabcdefabcdefabcdefabcdefabcdefabcd", "1")).isEqualTo(evm)
            assertThat(store.getEvmNftItem("0x1", "0xowner", "0xabcdefabcdefabcdefabcdefabcdefabcdefabcd", "1")).isEqualTo(evm)

            assertThat(store.observeSwtcNfts("jcccc").first()).hasSize(1)
            assertThat(store.observeEvmNftItems("0x1", "0xOWNER", "0xABCDEFABCDEFABCDEFABCDEFABCDEFABCDEFABCD").first()).hasSize(1)
            ...
            assertThat(database.nftDao().getItemCount("0x1", "0xowner", "0xabcdefabcdefabcdefabcdefabcdefabcdefabcd")).isEqualTo(1)
            ...
            assertThat(store.getEvmNftItem("0x1", "0xowner", "0xabcdefabcdefabcdefabcdefabcdefabcdefabcd", "1")).isNull()
        } finally {
            database.close()
        }
    }
    ...
    @Test
    fun create_with_context_initializes_room_backed_sdk() = runTest {
    ...
    @Test
    fun daoWrappers_roundTripAndDelete() = runTest {
        val database = newDatabase()
        val store = NftStore(database.nftDao())
        ...
        try {
            ...
            assertEquals(evm, store.getEvmNftItemByContractAndTokenId("0x1", "0xabcdefabcdefabcdefabcdefabcdefabcdefabcd", "1"))
            assertEquals(evm, store.getEvmNftItem("0x1", "0xowner", "0xabcdefabcdefabcdefabcdefabcdefabcdefabcd", "1"))
    ...
    @Test
    fun getAvatarCandidates_mapsSwtcAndEvmRows() = runTest {
        val database = newDatabase()
        val store = NftStore(database.nftDao())
    ...
    @Test
    fun resolveSwtcAvatar_prefersLocalMetaAndFallsBackToRow() = runTest {
        val database = newDatabase()
        val store = NftStore(database.nftDao())
    ...
    @Test
    fun resolveEthrAvatar_prefersLocalMetaAndFallsBackToEvmRow() = runTest {
        val database = newDatabase()
