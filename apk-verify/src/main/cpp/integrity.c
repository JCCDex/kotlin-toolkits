#include <jni.h>
#include <stdint.h>
#include <string.h>
#include <stdio.h>
#include <stdlib.h>

/* ── SHA-256 implementation (FIPS 180-4) ───────────────────────── */

#define SHA256_BLOCK_SIZE 64
#define SHA256_DIGEST_LEN 32
#define BUFFER_SIZE 8192

typedef struct {
    uint8_t  data[SHA256_BLOCK_SIZE];
    uint32_t datalen;
    uint64_t bitlen;
    uint32_t state[8];
} sha256_ctx;

static const uint32_t K[64] = {
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
    0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
    0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
    0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
    0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc,
    0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
    0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
    0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
    0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
    0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
    0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3,
    0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
    0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5,
    0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
    0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
    0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
};

#define ROTLEFT(a, b) (((a) << (b)) | ((a) >> (32 - (b))))
#define ROTRIGHT(a, b) (((a) >> (b)) | ((a) << (32 - (b))))
#define CH(x, y, z) (((x) & (y)) ^ (~(x) & (z)))
#define MAJ(x, y, z) (((x) & (y)) ^ ((x) & (z)) ^ ((y) & (z)))
#define EP0(x) (ROTRIGHT(x, 2)  ^ ROTRIGHT(x, 13) ^ ROTRIGHT(x, 22))
#define EP1(x) (ROTRIGHT(x, 6)  ^ ROTRIGHT(x, 11) ^ ROTRIGHT(x, 25))
#define SIG0(x) (ROTRIGHT(x, 7) ^ ROTRIGHT(x, 18) ^ ((x) >> 3))
#define SIG1(x) (ROTRIGHT(x, 17) ^ ROTRIGHT(x, 19) ^ ((x) >> 10))

static void sha256_transform(sha256_ctx *ctx, const uint8_t data[]) {
    uint32_t m[64], a, b, c, d, e, f, g, h, t1, t2;
    int i;
    for (i = 0; i < 16; i++)
        m[i] = ((uint32_t)data[i*4] << 24) | ((uint32_t)data[i*4+1] << 16) |
               ((uint32_t)data[i*4+2] << 8)  | data[i*4+3];
    for (i = 16; i < 64; i++)
        m[i] = SIG1(m[i-2]) + m[i-7] + SIG0(m[i-15]) + m[i-16];
    a = ctx->state[0]; b = ctx->state[1];
    c = ctx->state[2]; d = ctx->state[3];
    e = ctx->state[4]; f = ctx->state[5];
    g = ctx->state[6]; h = ctx->state[7];
    for (i = 0; i < 64; i++) {
        t1 = h + EP1(e) + CH(e,f,g) + K[i] + m[i];
        t2 = EP0(a) + MAJ(a,b,c);
        h = g; g = f; f = e; e = d + t1;
        d = c; c = b; b = a; a = t1 + t2;
    }
    ctx->state[0] += a; ctx->state[1] += b;
    ctx->state[2] += c; ctx->state[3] += d;
    ctx->state[4] += e; ctx->state[5] += f;
    ctx->state[6] += g; ctx->state[7] += h;
}

static void sha256_init(sha256_ctx *ctx) {
    ctx->datalen = 0; ctx->bitlen = 0;
    ctx->state[0] = 0x6a09e667; ctx->state[1] = 0xbb67ae85;
    ctx->state[2] = 0x3c6ef372; ctx->state[3] = 0xa54ff53a;
    ctx->state[4] = 0x510e527f; ctx->state[5] = 0x9b05688c;
    ctx->state[6] = 0x1f83d9ab; ctx->state[7] = 0x5be0cd19;
}

static void sha256_update(sha256_ctx *ctx, const uint8_t data[], size_t len) {
    size_t i;
    for (i = 0; i < len; i++) {
        ctx->data[ctx->datalen] = data[i];
        ctx->datalen++;
        if (ctx->datalen == SHA256_BLOCK_SIZE) {
            sha256_transform(ctx, ctx->data);
            ctx->bitlen += SHA256_BLOCK_SIZE * 8;
            ctx->datalen = 0;
        }
    }
}

static void sha256_final(sha256_ctx *ctx, uint8_t hash[]) {
    uint64_t i = ctx->datalen;
    ctx->data[i++] = 0x80;
    if (ctx->datalen >= 56) {
        memset(ctx->data + i, 0, SHA256_BLOCK_SIZE - i);
        sha256_transform(ctx, ctx->data);
        memset(ctx->data, 0, 56);
    } else {
        memset(ctx->data + i, 0, 56 - i);
    }
    ctx->bitlen += ctx->datalen * 8;
    for (i = 0; i < 8; i++)
        ctx->data[56 + i] = (ctx->bitlen >> (56 - i * 8)) & 0xFF;
    sha256_transform(ctx, ctx->data);
    for (i = 0; i < 4; i++) {
        hash[i]      = (ctx->state[0] >> (24 - i * 8)) & 0xFF;
        hash[4 + i]  = (ctx->state[1] >> (24 - i * 8)) & 0xFF;
        hash[8 + i]  = (ctx->state[2] >> (24 - i * 8)) & 0xFF;
        hash[12 + i] = (ctx->state[3] >> (24 - i * 8)) & 0xFF;
        hash[16 + i] = (ctx->state[4] >> (24 - i * 8)) & 0xFF;
        hash[20 + i] = (ctx->state[5] >> (24 - i * 8)) & 0xFF;
        hash[24 + i] = (ctx->state[6] >> (24 - i * 8)) & 0xFF;
        hash[28 + i] = (ctx->state[7] >> (24 - i * 8)) & 0xFF;
    }
}

static void bytes_to_hex(const uint8_t bytes[], size_t len, char *hex) {
    const char *digits = "0123456789abcdef";
    size_t i;
    for (i = 0; i < len; i++) {
        hex[i * 2]     = digits[(bytes[i] >> 4) & 0x0F];
        hex[i * 2 + 1] = digits[bytes[i] & 0x0F];
    }
    hex[len * 2] = '\0';
}

/* ── JNI: compute SHA-256 of a file, return lowercase hex ──────── */

JNIEXPORT jstring JNICALL
Java_com_jccdex_toolkits_apkverify_JniVerifier_nativeComputeSha256(
    JNIEnv *env, jclass clazz, jstring filePath)
{
    const char *path = (*env)->GetStringUTFChars(env, filePath, NULL);
    if (path == NULL) return NULL;

    FILE *fp = fopen(path, "rb");
    (*env)->ReleaseStringUTFChars(env, filePath, path);
    if (fp == NULL) return NULL;

    sha256_ctx ctx;
    sha256_init(&ctx);
    uint8_t *buf = (uint8_t *)malloc(BUFFER_SIZE);
    if (buf == NULL) { fclose(fp); return NULL; }

    size_t n;
    while ((n = fread(buf, 1, BUFFER_SIZE, fp)) > 0)
        sha256_update(&ctx, buf, n);

    free(buf);
    fclose(fp);

    uint8_t hash[SHA256_DIGEST_LEN];
    sha256_final(&ctx, hash);

    char hex[SHA256_DIGEST_LEN * 2 + 1];
    bytes_to_hex(hash, SHA256_DIGEST_LEN, hex);

    return (*env)->NewStringUTF(env, hex);
}

/* ── JNI: case-insensitive hash comparison ─────────────────────── */

JNIEXPORT jboolean JNICALL
Java_com_jccdex_toolkits_apkverify_JniVerifier_nativeHashEquals(
    JNIEnv *env, jclass clazz,
    jstring hash1, jstring hash2)
{
    if (hash1 == NULL || hash2 == NULL) return JNI_FALSE;

    const char *s1 = (*env)->GetStringUTFChars(env, hash1, NULL);
    const char *s2 = (*env)->GetStringUTFChars(env, hash2, NULL);
    if (s1 == NULL || s2 == NULL) {
        if (s1) (*env)->ReleaseStringUTFChars(env, hash1, s1);
        if (s2) (*env)->ReleaseStringUTFChars(env, hash2, s2);
        return JNI_FALSE;
    }

    size_t i;
    for (i = 0; s1[i] && s2[i]; i++) {
        char c1 = s1[i], c2 = s2[i];
        if (c1 >= 'A' && c1 <= 'F') c1 += 32;
        if (c2 >= 'A' && c2 <= 'F') c2 += 32;
        if (c1 != c2) {
            (*env)->ReleaseStringUTFChars(env, hash1, s1);
            (*env)->ReleaseStringUTFChars(env, hash2, s2);
            return JNI_FALSE;
        }
    }
    jboolean equal = (s1[i] == '\0' && s2[i] == '\0') ? JNI_TRUE : JNI_FALSE;

    (*env)->ReleaseStringUTFChars(env, hash1, s1);
    (*env)->ReleaseStringUTFChars(env, hash2, s2);
    return equal;
}
