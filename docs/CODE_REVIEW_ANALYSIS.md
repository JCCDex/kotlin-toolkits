...
| 维度 | 结论 |
| --- | --- |
| 安全 | 整体安全意识强（Tink + Argon2id + AAD + 密码 wipe + 锁定策略 + SSRF 防护 + origin 校验均有落地），但发现 **12 个高危**（部分同根）：① 账户导入链：先清库后查重（H-A1）、空私钥入库锁死真实私钥（H-A2）；② 桥接层：`postMessage` 信任边界缺失 + 私钥缓存窗（H-D1）、桥接回调可被页面 JS 伪造签名结果（H-W1）、私钥经 WebView 以不可擦除 String 传递（H-DID4）；③ 签名/批量：DApp 盲签凭证（H-DID1）、批量交易无上限（H-D2）；④ DID 缓存一致性：创建保护标志一次性失效导致文档误删（H-DID2）、删除后被链上旧文档复活（H-DID3）；⑤ 更新链路：证书校验 fail-open（H-W2）、无 HTTPS 强制/重定向降级（H-W3）、自校验信任根循环（H-W4）。另有中危若干：协程取消被吞、HTTP 无大小上限、`sendTransaction` 无逐笔确认、JNI 校验静默降级、SSRF 解析结果直返等 |
| 性能 | 主要问题集中在：HTTP 响应无大小上限（OOM 风险）、`CoroutineScope(...).launch` 大量 fire-and-forget、循环内 `Regex` 编译、Room 查询可合并、hex 编码低效 |
| 架构收敛 | **存在明确可收敛项**：`wallet.model.Path` 与 `core.model.Path` 完全重复；4 处几乎相同的 `HttpURLConnection` fetch 实现；2 套手写 hex 编码；JSON 处理在 org.json 与 Gson 之间混用；Room「Database 单例 + DAO + Store」样板在 account/did/nft 三模块重复；`ByteArray.wipe` 等安全工具仅存在于 vault 模块 |
...
- **问题**：`HttpURLConnection` 跟随跨 scheme 重定向（https→http）；调用方误传 `http://` 时 checksums 文件（含 `apkSha256`/`signingCertSha256`）与 APK 均可被 MITM 整体替换；checksums 文件本身无签名/公钥验证。
...
- **问题**：宿主以 `clearExisting=true` 重导同一助记词（用户忘记已导入、或 UI 文案误导）时，**先清空 vault 与账户表，再发现重复并返回错误**——既有钱包数据已不可逆销毁。即使未命中重复检查，用户也是在数据被清空后才得到确认机会。
...
- **位置**：
  ...
  - `account/.../orchestrator/AccountOrchestrator.kt:251`（`catch (e: Exception)`）、`:306`（`runOperation` 的 `catch (e: Exception)`）
  - `did/.../service/DidCoreService.kt:71`（`catch (_: Exception)`）
  - `did/.../service/DidSyncService.kt:20`（`runCatching { didSdk.resolveDid(did) }`）
  - `did/.../util/DidCredentialHelper.kt:31,81`、`did/.../sdk/DidSdk.kt:888,1098,1264,1289`（`runCatching` 包裹 suspend 调用）
...
- **位置**：
  ...
  - `vault/.../VaultRepository.kt` 全篇：密码/私钥以 `ByteArray` 传递并 wipe——**这是正确做法**；但 `account/.../AccountOrchestrator.kt:222` `mnemonic.toString(Charsets.UTF_8)`、`:103,124,239` 等把助记词/私钥转成不可变 `String` 再传给 `WalletSdk`（经 WebView JS），该 String 无法 wipe，只能等 GC。
...
- **位置**：
  ...
  - `nft/.../remote/SwtcChainNftClient.kt:81`（RPC 响应 `readText()`）
...
- **修复建议**：为签名/转账类方法增加可选的逐笔确认回调（与 `RequestAccountsCallback` 同构），或在 `WebAppInterface` 层对 `*_SENDTRANSACTION` 统一走确认流程；宿主侧务必在 `SecretProvider` 中实现逐笔确认。
...
- **位置**：`nft/.../storage/room/NftStore.kt:283`（完整元数据写入 DB）、`:497,516`（`fetchJson`/`fetchText` 的 `readText()`）、`NftRemoteAssetResolver.kt:182`、`SwtcChainNftClient.kt:81`。
...
#### M-10N：SWTC RPC 节点允许 `http://` + 跟随重定向 + 默认无证书固定

- **位置**：`nft/.../remote/SwtcChainNftClient.kt:26,74,93-99`。
- **问题**：`openPinnedConnection` 只对 `HttpsURLConnection` 且 `certificatePins` 非空时设置固定，默认 `certificatePins = emptyList()`；`postJson` 的 `instanceFollowRedirects = true` 且未校验重定向目标。中间人可把 RPC 响应替换为任意元数据 URI（后续解析仍过 SsrfGuard，但元数据内容本身可被篡改，影响 NFT 展示真实性）。
...
- **修复**：改为向当前页面真实 origin 投递（从 `webView.url` 解析），并在收到 JS 侧消息时校验来源。
...
#### M-16A：`importHdWallet` 触发双重 Argon2 KDF（性能 + 内存峰值）

- **位置**：`account/.../AccountOrchestrator.kt:100-106`（`vault.importMnemonic` 内部 `lockedImportPrivateKey` 已做一次 `derivedKey()` Argon2）与 `:144`（`vault.importPrivateKeys(keys)` 再做一次 `derivedKey()`）——若 vault 未解锁，两次都触发完整 Argon2 派生（64MB 内存 × 2，弱机明显卡顿）。
- **修复**：批量导入前先 `unlock` 一次并复用会话密钥；或 `importMnemonic` 与 `importPrivateKeys` 合并为单次会话内批量操作。
...
- **位置**：`nft/.../remote/SwtcChainNftClient.kt:110`（`filterIsInstance<X509TrustManager>().first()`，部分厂商 ROM 可能为空）。
...
- **修复**：`did` 加唯一索引 + `@Upsert` 或 `@Transaction` 保证原子性（与 M-5N 同类）。
...
- **位置**：`did/.../sdk/AndroidDidWebRuntime.kt:59-70`（构造即 `initialize()+start()` 创建隐藏 WebView 加载 ~12MB JS）、`DidSdk.kt:1310-1328`、`port/DidSdkPorts.kt:11-22`（`IDidBridge` 无 close）。
...
- **问题**：锁定期调用 `verifyPassword`（removeAccount/clearWalletData 路径）抛 `VaultAuthLockedException`，被 `catch (Exception)` 包装为 `AccountOperationError.Failure`——调用方无法区分「密码错误」与「账户锁定」，无法展示锁定倒计时（vault 提供 `authLockRemainingMs()` 但 orchestrator 未透传）。
...
- **问题**：`derivedKey()` 每次调用都 `copyOf()` 一份会话密钥，`changePassword` 在内存中同时存在旧 key、新 key、以及全部明文密钥/助记词的多份副本（`Vault.newBuilder()` 收集所有解密明文）。对于大钱包（1024 个地址上限）内存峰值可观；且 wipe 只能覆盖显式持有的数组，`Vault` builder 中的 `ByteString` 副本不可 wipe。
...
- **L-7**：`apk-verify/.../ApkIntegrityVerifier.kt:239` 临时 APK 文件写入 `context.cacheDir` 后 `finally { temp.delete() }`——若进程被杀，临时文件残留（cacheDir 会被系统清理，风险低）。建议文件名加随机后缀避免并发覆盖冲突。
...
- **L-14**：`DidRoomDatabase.exportSchema = false`（`DidRoomDatabase.kt:14`）阻碍未来迁移验证（与 nft 的 M-5N 同族），建议开启 schema 导出。
...
- **L-23**：`apk-verify/.../ReleaseChecksumsFile.kt:23-37` 与 `OfficialReleaseManifest.kt:39,51-62`：`readText()` 无大小上限（大文件 OOM）、未显式指定字符集（依赖平台默认）；`signingCertSha256`/`apkSha256` 无 hex 格式校验（非 hex 值仅比较时恒不等，报错语义差）。解析逻辑本身（hex 正则 + `toIntOrNull`）校验严格，是好的部分。
...
`NftRemoteAssetResolver.fetchMetadataImage`、`NftStore.fetchJson/fetchText`、`SwtcChainNftClient.postJson`、`AppUpdateChecker.fetchText`、`AppUpdateApkInstaller.downloadAndVerify` 是 **5 处几乎相同的 `HttpURLConnection` 样板**（connect/read 超时、responseCode 判断、`readText()`、`disconnect()`）。差异仅在：是否 SSRF 检查、是否重定向、读文本还是写文件。收敛为 core 的 `HttpFetcher`（参数：URL、超时、大小上限、SSRF 开关、重定向策略）后：
...
`vault/.../Argon2idKdf.kt:56-62`：`when { mc <= 256 -> 64 to 3; ... }` 的 magic number 建议提取为命名常量并注释推导依据（内存 1/4 规则、迭代 3/2/2）。

#### P-8：Room 查询合并

- `account/.../RoomAccountStore.kt:139` `getCurrentAccountIdSync()` 每次 `suspend` 调用都查一次库；`AccountSdk` 的 `accounts` Flow 在 orchestrator 中被反复 `.first()`——若出现「一次操作多次 `.first()`」的热点（如 `requestAccounts` 内 `accountProvider.accounts.first()`），可考虑在 Provider 层做一次缓存快照。
...
- **位置**：`nft/.../storage/room/NftStore.kt:321-330`（`resolveCredentialImages` 对请求列表顺序执行）、`NftRemoteAssetResolver.kt`（`resolveSwtcAvatar` 最多 4 阶段串行）。
...
- **问题**：单次 RPC 调用内多次查询账户表（如 `sendTransaction` 一次调用触发 2~3 次）；批量场景放大。账号多时是明显开销。
...
- **位置**：`did/.../sdk/DidSdk.kt:509-584`（`updateDidAvatar` 同一文档至少 4 次 `JSONObject(doc)`，`:560` 还 `toString()` 后再 parse）、`:1080-1089`（每次更新额外一次 JS 桥 `didStat`）。
- **修复**：解析一次传对象；避免 `json.toString()` 后再解析；本地缓存 cid；`did-bridge` 加载的 ~12MB JS（`did-0.3.2.min.js` 9.6MB）评估懒加载/按需拆分。
...
`core`（`com.jccdex.toolkits.core.model`）仅包含 `ChainType`、`Path`、`WalletAccount` 三个模型 + `toBip44JsonArray` 扩展。**尚无任何工具层**（无 string/JSON/编码/网络/异常工具），这是收敛的天然落点。core 目前零第三方依赖、minSdk 26，非常适合承载纯 Kotlin 工具。
...
| C-2 | **HTTP fetch 样板 ×5** | `NftRemoteAssetResolver.kt`、`NftStore.kt`（×2）、`SwtcChainNftClient.kt`、`AppUpdateChecker.kt`、`AppUpdateApkInstaller.kt` | core 新增 `HttpFetcher`（超时、大小上限、SSRF 开关、重定向、流式读写） |
...
| C-14 | **JSON 点路径读取器（逐行相同）** | `did/.../sdk/DidSdk.kt:1152-1179`（`readString`/`readElement`，Gson）与 `nft/.../storage/room/NftStore.kt:416-435`（`parseString`，同款 Gson 点路径） | core `core.json.JsonPath`（统一 Gson/org.json 策略） |
| C-15 | **DID 文档解析/凭证查找/VC ID 生成** | `DidSdk.readServices/readProfileField/readJsonArray`、`DidCoreService.readProfileField`、`DidCredentialHelper.readCredentials`；`DidSdk.findCredentialById` 与 `DidCredentialHelper.findCredentialIndex` 重复；`DidSdk.buildAvatarCredentialId` 与 `DidCredentialHelper.generateVcId` 同构 | core（或 did 模块内收敛为单一实现），消除同模块内部重复 |
| C-16 | **NFT 标准常量大小写不一致** | `DidSdk.kt:1307-1308`（`"jingtumnft"`/`"erc-721"`）vs `DidCredentialHelper.kt:15-16`（`"jingtumNFT"`/`"ERC-721"`） | core 常量（`core.nft.Standards`），消除隐式字符串协议 |
...
| C-21 | **NFT 元数据解析合并** | `SwtcNftMetadataParser.extractMetadataFields` 与 `NftRemoteAssetResolver.extractMetadataImageUrl` 都做「JSON → image/name/description」提取 | core `NftMetadataParser`（含 `JsonObject` 重载，一并修复 P-14N 双重解析） |
| C-22 | **「非根账户」判定 SQL 与 core 模型语义不一致** | `AccountDao.getNonRootAccount` 的 SQL 谓词 `(isHD=1 AND parentId IS NOT NULL) OR isHD=0`（`AccountDao.kt:42`）vs core `WalletAccount.isSubHD()`（`core/model/WalletAccount.kt:21`）——`isHD=true, parentId=null, path 非 root` 时 SQL 判「根」、core 判「子」 | 非根判定收敛到 core 单一来源（SQL 侧生成或引用同一常量/表达式），消除两套语义 |
| C-23 | **JS 引号转义 / SWTC 金额币种正则 / DAppMethod 映射** | `DAppConnectSdk.jsQuote`（`:132-133`）；`SwtcBatchTransactions.kt:17,96,118` 的金额/currency-issuer 正则；`WebAppInterface.postMessage` 的 `when(method)` 20 分支分发 | core `core.js.JsQuote`（含 `\u2028/\u2029` 转义）、`core.swtc.Regexes`、`core.rpc.MethodDispatcher`（方法→handler 映射表替代巨型 when） |
| C-24 | **wallet 桥接双接口近乎重复** | `wallet/.../AndroidWalletWebRuntime.kt:9-33`（`IWalletWebBridgeClient`）与 `:65-84`（`IWalletBridge`）字段与语义几乎一致 | 合并为单一接口（或统一由 core 桥接抽象提供），消除 2×N 方法签名维护成本 |
...
- **B-5**：`!!` 非空断言残留：`DidCredentialHelper.kt:75,87`（`data.restrictions!!`）——`buildNftSubject` 未先调用 `validateCredentialData`，`!!` 处可能 NPE。应改为 `requireNotNull(data.restrictions)` 并给出清晰 message，或在调用链上先验证。
- **B-6**：`VaultRepository.getBiometric()` 抛 `Error`（见 L-1）——业务失败必须用异常类型而非 `Error`。
- **B-7**：`runCatching` 的使用（did/nft/apk-verify/app-update 共 14 处）多数是「解析容错」场景（合理），但包裹 suspend 调用时吞掉 `CancellationException`（见 M-1）。建议 core 提供 `safeRunCatching`（内部先重抛取消）。
...
- **B-9**：`WebAppInterface.kt` 24 处 `CoroutineScope(Dispatchers.IO).launch { }` fire-and-forget：无 Job 管理、无生命周期绑定、无取消。DApp 页面跳转/WebView destroy 后协程仍运行，回调 `NativeResponseChannel` 时 WebView 已销毁（已有 try/catch 兜底但会打日志噪音）。建议：注入统一 `CoroutineScope`（宿主生命周期），或改用 `SupervisorJob` + 显式 `cancel` 钩子。
...
- **B-16**：魔法数字：`EthMiddleware.kt:319-320` `"0x5208"`（21000 gas）、`VaultAuthLockout.kt:71-74`（锁定时长）、`Argon2idKdf.kt:56-62`（内存参数）、`WebviewBridgeClient.kt:199` `30_000L` 超时——建议命名常量 + 注释（vault 的常量命名已很好，保持）。
...
- **B-21**：布尔返回值 API 无法表达失败原因：`DidSdk` 的 `uploadInitialDidDoc/updateDidNickname/updateDidAvatar/publishDidDelete` 等 10 处返回 `Boolean`，链上拒绝/网络错误/参数非法无法区分（`DidWriteResult` 已是正确示范，应推广）。
- **B-22**：函数名与副作用不符：`DidCoreService.resolveAndSaveDid` 名称暗示只读解析，实际写库并修改 pending 状态（`DidSdk.nickname(doc)` 实际是「提取昵称」，建议 `extractNickname`）。命名应暴露副作用。
...
- **B-27**：`AppUpdateApkInstaller.downloadAndVerify` 7 个参数 → 提取 `DownloadRequest` data class；`AppUpdateCheckThrottle.force: Boolean` 无默认值且语义含糊 → `enum ForcePolicy`。
...
- **B-31**：`DidRoomDatabase.exportSchema = false` 阻碍未来迁移验证，建议开启并纳入版本控制；`NftRoomDatabase` 无迁移策略（见 M-11N）。
...
- **B-41**：account 错误类型风格不一致：`AccountOperationError.WrongPassword` 是 `data class`（带默认 message）而其余是 `data object`（`AccountOperationError.kt:4-17`）；`AccountOperationResult.Error` 与 `kotlin.Error` 同名易混淆。
...
- **B-47**：`WebAppInterface.postMessage` 的 `when(method)` 20 个分支样板重复（每个 handler 都做「取 params → 校验长度 → 调 middleware → 发响应」）——可收敛为「方法 → handler 注册表」模式（C-23），同时天然获得统一参数校验与错误映射。
...
2. **M-3 / M-8N / M-5W**：HTTP 响应统一大小上限（core `HttpFetcher`）。
...
1. **C-2/C-13/C-17**：core 新增 `HttpFetcher` / `Hashing` / `SecureCompare` / `Hex`（消除 5 处 HTTP 样板、2 套 hex、双哈希路径）。
2. **C-14/C-15/C-16/C-21**：JSON 点路径读取器、DID 文档解析、VC ID 生成、NFT 标准常量与元数据解析收敛 core。
3. **C-22**：「非根账户」判定 SQL 与 core `isSubHD()` 收敛为单一来源。
4. **M-4 / H-D4**：签名/转账类 RPC 统一逐笔确认回调；宿主 SecretProvider 逐笔确认。
...
9. **X-1 / X-2 / X-4**：统一桥接运行时——合并 wallet/did 双 WebView、消除 `WebviewBridgeEngine`/`JsPromiseGateway` 死公共 API、明确 callbackMap 隔离契约。

---

## 8. Pro 复核（二次对抗式分析 + 跨模块补充）

> 第二轮用 deepseek-v4-pro 对首轮报告中**依赖子代理、未亲自逐行核实**的高价值发现做了对抗式核验，并补做模块级审查易遗漏的**跨模块/架构级**分析。结论：已核验发现**全部与源码一致，无幻觉**；并新增 4 项跨模块发现。
...
- **位置**：`wallet/.../AndroidWalletWebRuntime.kt:36`（`WebviewBridgeClient()` 加载 `wallet-bridge.html`）、`did/.../AndroidDidWebRuntime.kt:34`（`WebviewBridgeClient()` 加载 `did-bridge.html` ~9.6MB JS）。
- **问题**：完整集成（account + vault + wallet + did + dapp-connect）时存在 **2 个独立隐藏 WebView**（wallet 一个、did 一个），各占一个 renderer 进程、各加载 10+ MB JS。这意味着：(a) H-DID4/C-04 的「私钥进入 WebView 进程」风险被**翻倍**——钱包侧和 DID 侧是两个独立 JS 上下文，攻击面不重叠但均暴露；(b) 内存常驻（两个 renderer 进程 + 两份 JS 堆）；(c) 生命周期管理各自为政（`destroy()` 各调各的）。
...
- **位置**：`webview-bridge/.../WebviewBridgeEngine.kt:6-35`（object 门面）、`JsPromiseGateway.kt:83-115`（object 单例）。
...
- **位置**：`JsPromiseGateway.kt:83-115`（`object` 委托 `PromiseGatewayImpl`）、`WebviewBridgeClient.kt:25-30`（构造时若传 `JsPromiseGateway` 则共享其 `callbackMap`）。
